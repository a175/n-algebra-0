...
\begin{remark}
  ...
  また, 例えば, 群$(G,\ast,e,\tofu^{-1})$であれば,
  正の整数$n$に対し,
  \begin{align*}
    x^n &= (\cdots((x\ast x)\ast x) \ast \cdots) \ast x\\
  \end{align*}
  のように書く.
  群の演算は結合律が成り立つので,
  $x^n$は括弧の付け方を変えても同じものになる.
  また, $x^0=e$とし, $x^{-n}=(x^{-1})^n$とする.
  このとき, いわゆる指数法則
  \begin{align*}
    x^{n}\ast x^{m} &= x^{n+m}\\
    (x^{n})^{m} &= x^{nm}
  ...
  その演算を和と呼ぶ.
  また, 例えば, 加法群$(G,+,o,-\tofu)$では,
  正の整数$n$に対し,
  \begin{align*}
    nx &= (\cdots((x+ x)+ x)+ \cdots) +x \\
  \end{align*}
  のように書く.
  また, $0x=o$とし, $(-n)x=n(-x)$とする.
  このとき, いわゆる分配法則
  \begin{align*}
    nx+m x &= (n+m)x\\
    m(nx) &= (nm)x
